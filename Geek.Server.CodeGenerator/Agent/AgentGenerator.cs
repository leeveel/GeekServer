...
namespace Geek.Server.CodeGenerator.Agent
{
    ...
    public class AgentGenerator : ISourceGenerator
    {
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            {
                GenLocalAgent(context, receiver);
                GenRemoteAgent(context, receiver);
            }
        }

        void GenLocalAgent(GeneratorExecutionContext context, AgentFilter receiver)
        {
            //生成模板
            ...
            foreach (var agent in receiver.AgentList)
            {
                ...
                foreach (var member in agent.Members)
                {
                    if (member is MethodDeclarationSyntax method)
                    {
                        ...
                        {
                            mth.ParamTypes.Add(p.Type.ToString());
                            mth.Params.Add(p.Identifier.Text);
                        }

                        if (mth.IsPublic && mth.Isreturntask)
                            info.Publictaskmethods.Add(mth);
        ...
        void GenRemoteAgent(GeneratorExecutionContext context, AgentFilter receiver)
        {
            var ingoreFuncs = new List<string> {
                "ActorId",
                "Owner",
                "Active",
                "Deactive",
            ...
            //生成模板
            var str = ResLoader.LoadTemplate("RemoteAgent.liquid");
            Template agentTemplate = Template.Parse(str);
            foreach (var agent in receiver.AgentList)
            {
                string fullName = agent.GetFullName();
                AgentInfo info = new AgentInfo();
                info.Super = agent.Identifier.Text;
                info.Name = info.Super + "RemoteWrapper";
                //info.Space = Tools.GetNameSpace(fullName);
                ...
                foreach (var member in agent.Members)
                {
                    if (member is MethodDeclarationSyntax method)
                    {
                        if (ingoreFuncs.Contains(method.Identifier.Text))
                        {
                            continue;
                        }
