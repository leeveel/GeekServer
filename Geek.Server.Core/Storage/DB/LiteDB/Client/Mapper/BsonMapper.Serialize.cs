...
namespace LiteDB
{
    public partial class BsonMapper
    {
        ...
        //多态通过记录真实类型id来
        internal BsonValue Polymorphic(Type type, BsonValue value)
        {
            if (typeToIdGetter == null || value == null || value is not BsonDocument)
                return value;
            var doc = value as BsonDocument;
            if (typeToIdGetter(type, out var id))
            {
                doc["_t"] = id;
            }
            return doc;
        ...
        internal BsonValue Serialize(Type type, object obj, int depth)
        {
            ...
            {
                return Polymorphic(type, custom(obj));
            }

            var typeInfo = type.GetTypeInfo();
            if (typeInfo.IsGenericType && !typeInfo.ContainsGenericParameters && _customSerializer.TryGetValue(typeInfo.GetGenericTypeDefinition(), out custom))
            {
                return Polymorphic(type, custom(obj));
            }

            var objTypeInfo = obj.GetType().GetTypeInfo();
            if (objTypeInfo.IsGenericType && !objTypeInfo.ContainsGenericParameters && _customSerializer.TryGetValue(objTypeInfo.GetGenericTypeDefinition(), out custom))
            {
                return Polymorphic(type, custom(obj));
            }
            ...
            else if (obj is Int32 v) return new BsonValue(v);
            ...
            {
                return Polymorphic(type, this.SerializeArray(Reflection.GetListItemType(type), obj as IEnumerable, depth));
            }
            // otherwise serialize as a plain object
            else
            {
                return Polymorphic(type, this.SerializeObject(type, obj, depth));
            }
        }
