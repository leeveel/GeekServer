...
namespace LiteDB
{
    public partial class BsonMapper
    {
        ...
        internal Type Polymorphic(BsonValue value)
        {
            if (idToTypeGetter == null || value == null || value is not BsonDocument)
                return null;
            var doc = value as BsonDocument;
            var idValue = doc["_t"];
            if (idValue == BsonValue.Null)
                return null;
            var id = idValue.AsInt32;
            if (idToTypeGetter(id, out var t))
            {
                return t;
            }
            return null;
        ...
        public object Deserialize(Type type, BsonValue value)
        {
            ...
            if (typeInfo.IsGenericType && !typeInfo.ContainsGenericParameters && _customDeserializer.TryGetValue(typeInfo.GetGenericTypeDefinition(), out custom))
            {
                return custom(type, value);
            }
            ...
            else if (value.IsDocument)
            {
                ...
                if (false && doc.TryGetValue("_type", out var typeField) && typeField.IsString) //(doc.TryGetValue("_type", out var typeField) && typeField.IsString)) 类型通过多态处理
                {
                    var actualType = _typeNameBinder.GetType(typeField.AsString) ?? throw LiteException.InvalidTypedName(typeField.AsString);
