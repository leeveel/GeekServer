...
namespace LiteDB
{
    ...
    public partial class BsonMapper
    {
        ...
        private readonly ConcurrentDictionary<Type, Func<Type, BsonValue, object>> _customDeserializer = new ConcurrentDictionary<Type, Func<Type, BsonValue, object>>();

        //如果想兼容mongodb的ignore属性，可以注册

        private readonly ConcurrentBag<Type> _customIgnoreAttrs = new ConcurrentBag<Type>();
        ...
        public delegate bool TryGetTypeById(int id, out Type t);
        public delegate bool TryGetIdByType(Type type, out int id);

        public TryGetTypeById idToTypeGetter;
        public TryGetIdByType typeToIdGetter;
        ...
        public BsonMapper(Func<Type, object> customTypeInstantiator = null, ITypeNameBinder typeNameBinder = null)
        {
            ...
            RegisterType<Uri>(uri => uri.IsAbsoluteUri ? uri.AbsoluteUri : uri.ToString(), (t, bson) => new Uri(bson.AsString));
            RegisterType<DateTimeOffset>(value => new BsonValue(value.UtcDateTime), (t, bson) => bson.AsDateTime.ToUniversalTime());
            RegisterType<TimeSpan>(value => new BsonValue(value.Ticks), (t, bson) => new TimeSpan(bson.AsInt64));
            RegisterType<Regex>(
                r => r.Options == RegexOptions.None ? new BsonValue(r.ToString()) : new BsonDocument { { "p", r.ToString() }, { "o", (int)r.Options } },
                (t, value) => value.IsString ? new Regex(value) : new Regex(value.AsDocument["p"].AsString, (RegexOptions)value.AsDocument["o"].AsInt32)
            );

            RegisterType(typeof(KeyValuePair<,>), value =>
            {
             ...
             (type, bsonValue) =>
             {
                 ...
                 var k = Deserialize(typeArgs[0], doc["k"]);
                 var v = Deserialize(typeArgs[1], doc["v"]);
                 return Activator.CreateInstance(type, k, v);
             });

            RegisterType<DateTime>(obj =>
            {
                var doc = new BsonDocument
                {
                    ["t"] = obj.ToUniversalTime().Ticks,
                    ["k"] = (int)obj.Kind
                };
                return doc;
            },
            ...
            {
                if (doc.IsDateTime)
                    return doc.AsDateTime;
                return (DateTimeKind)doc["k"].AsInt32 switch
                {
                    DateTimeKind.Utc => new DateTime(doc["t"].AsInt64, DateTimeKind.Utc),
                    DateTimeKind.Local => new DateTime(doc["t"].AsInt64, DateTimeKind.Local).Add(TimeZoneInfo.Local.BaseUtcOffset),
                    DateTimeKind.Unspecified => new DateTime(doc["t"].AsInt64, DateTimeKind.Unspecified),
                    _ => new DateTime(doc["t"].AsInt64, DateTimeKind.Unspecified),
                };
            }
            );
        ...
        public void RegisterIgnoreAttribute(Type t)
        {
            _customIgnoreAttrs.Add(t);
        }

        /// <summary>
        /// Register a custom type serializer/deserialize function
        /// </summary>
        public void RegisterType<T>(Func<T, BsonValue> serialize, Func<Type, BsonValue, T> deserialize)
        {
            _customSerializer[typeof(T)] = (o) => serialize((T)o);
            _customDeserializer[typeof(T)] = (t, b) => (T)deserialize(t, b);
        }

        /// <summary>
        /// Register a custom type serializer/deserialize function
        /// </summary>
        public void RegisterType(Type type, Func<object, BsonValue> serialize, Func<Type, BsonValue, object> deserialize)
        {
            _customSerializer[type] = (o) => serialize(o);
            _customDeserializer[type] = (t, b) => deserialize(t, b);
        }
        ...
        protected virtual EntityMapper BuildEntityMapper(Type type)
        {
            ...
            foreach (var memberInfo in members)
            {
                ...
                foreach(var ignoreA in _customIgnoreAttrs)
                {
                    if (CustomAttributeExtensions.IsDefined(memberInfo, ignoreA, true)) continue; 
                }
